...
### 
import id3_Valor_pokes
import id3_Atributo_pokes
import id3_Nodo_pokes
import math
contadorNodos = 0
####################################################################################################
def generarVector(listaAtributos, vector):    #Vector = registro/caso/fila
    ...
    for i in range(len(vector)):
        aux.append(id3_Valor_pokes.Valor(listaAtributos[i].nombre, vector[i]))
    return aux
####################################################################################################
...
def removerAtributo(caso, nombre):
    ...
    for j in range(len(caso)): # Por cada caso        
        if id3_Valor_pokes.Valor(nombre) in caso[j]:
            indiceColumna = caso[j].index(id3_Valor_pokes.Valor(nombre))
...
def ID3(casos, listaAtributos, mayoritaria):
    ...
    contadorNodos+=1
    root = id3_Nodo_pokes.Nodo(contadorNodos)
    mismaClase = False
    aux = -1
    clase = listaAtributos[len(listaAtributos) - 1];
    for k in range(len(clase.listaEtiquetas)): # por cada clase
...
####################################################################################################
#EJECUCIÓN PRINCIPAL
archivo = open("pre-procesados/pokes_discret_train.csv")
...
for i in range(len(aux)):
    listaAtributos.append(id3_Atributo_pokes.Atributo(aux[i]))
...
casos = []
try:        
    arch = open("pre-procesados/pokes_discret_test.csv")
