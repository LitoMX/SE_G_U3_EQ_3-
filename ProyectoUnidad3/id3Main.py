...
def generarVector(listaAtributos, vector):    #Vector = registro/caso/fila
    ...
    for i in range(len(vector)):
        aux.append(id3Valor.Valor(listaAtributos[i].nombre, vector[i]))
    return aux
####################################################################################################
...
def removerAtributo(caso, nombre):
    ...
    for j in range(len(caso)): # Por cada caso        
        if id3Valor.Valor(nombre) in caso[j]:
            indiceColumna = caso[j].index(id3Valor.Valor(nombre))
...
def ID3(casos, listaAtributos, mayoritaria):
    ...
    contadorNodos+=1
    root = id3Nodo.Nodo(contadorNodos)
    mismaClase = False
    aux = -1
    clase = listaAtributos[len(listaAtributos) - 1];
    for k in range(len(clase.listaEtiquetas)): # por cada clase
...
def id3():
    #EJECUCIÓN PRINCIPAL
    archivo = open("pre-procesados/pokes_discret_train_proyectoU3.csv")
    ...
    for i in range(len(aux)):
        listaAtributos.append(id3Atributo.Atributo(aux[i]))
    ...
    casos = []
    try:
        arch = open("pre-procesados/pokes_discret_test_proyectoU3.csv")
