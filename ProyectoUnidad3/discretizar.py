...
def guardarCSV(discretizado,nomyclase):
    with open('pre-procesados/pokes_discretizado_proyectoU3.csv', 'w', newline='') as archivo:
        writer = csv.writer(archivo, delimiter=',')
        #writer.writerow(cab)
        for n, d in zip(nomyclase, discretizado):
            writer.writerow([d[0], d[1], d[2], d[3], d[4],n])

def main_discretizar(vProblema):
    clas, arch = leerArchivo(vProblema)
    inter = intervalos(arch, 5)
    discre = discretizar(arch, 5, inter)
    #print(discre)
    guardarCSV(discre, clas)
    #print("discretizado listo")

if __name__ == "__main__":
    main_discretizar([0,0,0,0,0])
