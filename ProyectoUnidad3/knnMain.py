...
def knn(vProblema):
    ...
    archivo = open("pre-procesados/pokes_train.csv","r")
    ...
    instancia = [ [ list(map(float,x[:5])), x[5] ] for x in lista ] #iris
    ...
    archivo = open("pre-procesados/pokes_test.csv","r")
    contenido = archivo.readlines()
    ...
    prueba = [ [ list(map(float,x[:5])), x[5] ] for x in lista ] #iris

    prueba.append([vProblema," "])
